...
MainForm::MainForm()
{
	...
	departmentView->setCurrentIndex(
								departmentModel->index(0, 0));
}

//  QUANDO DEPARTAMENTO MUDAR, O  SLOT ABAIXO
// IRÁ MUDAR A VIEW DE EMPREGADOS:
...
void MainForm::addDepartment()
{
	...
	QModelIndex index =
							departmentModel->index(row, Department_Name);
...
void MainForm::deleteDepartment()
{
    ...
    departmentModel->submitAll();
	 QSqlDatabase::database().commit(); // encerra transaction
...
void MainForm::createDepartmentPanel()
{
	...
	departmentView->setSelectionBehavior(
