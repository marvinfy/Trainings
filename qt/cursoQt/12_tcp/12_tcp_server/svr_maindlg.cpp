...
void svr_mainDlg::on_stopButton_clicked()
{
 ...
 /*   foreach ( QObject * obj , tcpServ.children())
    {
		  TcpDataExchangeSocket * sock =
											qobject_cast<TcpDataExchangeSocket *>(obj);
        if ( sock )
                sock->close(); // interrompe comunicação com este cliente.
    }
	 */
	 QList <TcpDataExchangeSocket *> sockets =
								 tcpServ.findChildren<TcpDataExchangeSocket *>();

	 foreach(TcpDataExchangeSocket *sock, sockets)
		 sock->close();
}
