	...
	: QLCDNumber(parent)
{
	...
	// conecta o signal "timeout" do objeto timer (será emitido sempre que o intervalo de tempo for atingido)
...
void DigitalClock::showTime()
{
	...
	// converte o objeto QTime para string, com o formato especificado no argumento:
	QString text = time.toString("hh:mm:ss");
	
	display(text); // exibe o texto contendo a hora atual no QLCDNumber

	// emite o sinal "timeChanged", passando o texto como argumento:
	emit timeChanged(text);
	// e outros objetos desta aplicação poderão ter um slot conectado a este signal
}
