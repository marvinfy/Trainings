...
{
   // QSqlDatabase db1 = QSqlDatabase::addDatabase("QSQLITE", "dbSQLITE");
	// QSqlDatabase db2 = QSqlDatabase::addDatabase("QOCI", "dbORACLE");
	// db1.setDatabaseName(...);
	//db1.setHostName(...); user... password...
	// db1.open();
	// db2 - idem (names, password...)
	// db2.open();

//	QSqlQuery query( QSqlDatabase::database("dbORACLE") ) ;
...
/*
	// prepare no estilo ODBC:
	query.prepare("INSERT INTO employee (name, departmentid, "
				  "extension, email, startdate) "
				  "VALUES (?,?,?,?,?)");
	foreach (QString name, names) {
		query.addBindValue(name);
		query.addBindValue( 1 + (std::rand() % 5));
		query.addBindValue( 400 + (std::rand() % 100));
		query.addBindValue( name.toLower().replace(" ", ".") +
								  "@company.com");
		query.addBindValue(QDate::currentDate().addDays(-(std::rand() % 3600)));
		query.exec();
		++count;
		progress.setValue(count + 6);
	}
