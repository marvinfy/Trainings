...
private:
	...
	public:
		...
		void doTask () {  // irá "acordar" o thread que aguarda nesse semáforo na função "run"
			waitForTask.release(1) ;
		}
		void stop( ) {  // encerra o loop da função "run"
			stopped = true;
			doTask();
		}
	...
	protected:
		void run()// para a aplicação o thread começa a rodar aqui
		{
			...
			while (!stopped )
			{
				// aguarda indefinidamente até que uma terefa seja solicitada:
				...
				if ( !stopped )
				{
					...
					{
						// hummm... não vou usar um mutex aqui...
						sum += qrand(); // ... será que "qrand" é "thread-safe"??? ...
					}

					++times;
					QString result = QString( "Execucão #%0 - resultado=[%1]").arg(times).arg(sum) ;
